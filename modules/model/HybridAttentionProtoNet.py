...
class InstanceAttention(nn.Module):
    ...
    def forward(self, support, query):
        # support/query shape: [qk*n*n*k, d]
        k = self.k
        qk = self.qk
        n = self.n
        d = support.size(1)
        ...
        query = self.g(query)
        # shape: [qk*n, n, k, d]->[qk*n, n, k]
        attentions = t.tanh(t.mul(support, query).view(qk*n, n, k, d)).sum(dim=3).squeeze()
        # shape: [qk*n,n,k]->[qk*n,n,k,d]
        attentions = t.softmax(attentions, dim=2).unsqueeze(3).repeat(1,1,1,d)

        return t.mul(attentions, support.view(qk*n, n, k, d))
...
class HAPNet(nn.Module):
    def __init__(self, input_size, n, k, qk, encoder_depth=4):
        ...
        embed_size = 64#get_embed_size(input_size, encoder_depth)
        self.d = embed_size
        # self.d = embed_size
    ...
    def forward(self, support, query):
        ...
        assert qk*n==query.size(0), "qk与实际输入的query长度不一致"
        ...
        feature_attentions = self.FeatureAttention(support.unsqueeze(dim=1)).squeeze().repeat(qk*n,1,1)
        ...
        support_expand = support.repeat((qk*n,1,1,1)).view(qk*n*n*k,d)
        query_expand = query.repeat((n*k,1,1)).transpose(0,1).contiguous().view(qk*n*n*k,d)
        ...
        # proto shape: [qk*n,n,k,d]->[qk*n,n,d]
        support = support.sum(dim=2).squeeze()

        # query shape: [qk*n,d]->[qk*n,n,d]
