...
class SNAIL(nn.Module):
    ...
    def forward(self, support, query, s_labels):
        ...
        # batch_size = batch_length//(self.K*self.N+1)
        # x = self.encoder(input).view(batch_length, -1)
        ...
        x = torch.cat((x, labels), 2)       # 因为标签是one-hot编码，与标签混入时维度加N
