...
DOC_SAVE_PATH = "D:/Few-Shot-Project/doc/dl_RoutingNet_exp/"
...
for episode in range(MAX_ITER):
    ...
    if (episode + 1) % 5000 == 0:
        ...
        t.save(net.state_dict(),
               MODEL_SAVE_PATH + "RoutingNet_%d_epoch_model_%dshot_%dway_v%d.0.h5" % (episode + 1, k, n, version))
    ...
    if episode % TEST_CYCLE == 0:
        ...
        with no_grad():
            ...
            if test_acc/TEST_EPISODE > best_acc:
                t.save(net.state_dict(),
                       MODEL_SAVE_PATH + "RoutingNet_best_acc_model_%dshot_%dway_v%d.0.h5" % (k, n, version))
...
plt.title('%d-shot %d-way Routing Net Accuracy'%(k,n))
...
plt.title('%d-shot %d-way Routing Net Loss'%(k,n))
