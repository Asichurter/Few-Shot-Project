    ...
    if episode % TEST_CYCLE == 0:
        ...
        with no_grad():
            ...
            if test_acc/TEST_EPISODE > best_acc:
                t.save(net.state_dict(),
                       MODEL_SAVE_PATH + "ChannelNet_best_acc_model_%dshot_%dway_v%d.0.h5" % (k, n, version))
...
plt.title('%d-shot %d-way Channel Net Accuracy'%(k,n))
...
plt.title('%d-shot %d-way Channel Net Loss'%(k,n))
