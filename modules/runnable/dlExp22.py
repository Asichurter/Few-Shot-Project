...
from modules.utils.dlUtils import net_init, RN_labelize, labels_one_hot, labels_normalize
...
for episode in range(MAX_ITER):
    ...
    # sample_labels = labels_normalize(sample_labels, n, batch_length=k*n+1)
    ...
    outs = net(samples, queries, sample_labels)[:, -1, :].squeeze() # 取一个batch的最后一个输出
                                                                # 因为batch的最后一个是测试样本
    ...
    if episode % TEST_CYCLE == 0:
        ...
        with no_grad():
            ...
            for j in range(TEST_EPISODE):
                ...
                test_relations = net(supports, tests, support_labels)[:, -1, :].squeeze()
...
plt.title('%d-shot %d-way SNAIL Accuracy'%(k,n))
...
plt.title('%d-shot %d-way SNAIL Loss'%(k,n))
