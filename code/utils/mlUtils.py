...
import numpy as np
import random as rd
from code.utils.extract import extract_infos
...
MALWARE_BASE = r'D:/pe/'
#良性文件路径存储文件
BENIGN_PATH = "D:/Few-Shot-Project/data/benign_0730.npy"
...
def get_malwares(mal_base=MALWARE_BASE, each_num=100, targets=None):
    assert targets is None or type(targets)==list, "targets对象类型无效1"
    data = []
    for ex_i, mal_type in enumerate(os.listdir(mal_base) if targets is None else targets):
        if mal_type != 'aworm':
            # 采取在文件夹内抽样的形式，而非按顺序抽取
            samples = rd.sample(os.listdir(str(mal_base + mal_type)), each_num)
            ...
            for child_dir in os.listdir(str(mal_base + mal_type)):
                samples = rd.sample(os.listdir(str(mal_base + mal_type + '/' + child_dir)), each_num)
                for in_i, mal_name in enumerate(samples):
...
def get_benigns(num, benign_path=BENIGN_PATH):
    benign_paths = np.load(BENIGN_PATH).tolist()
    assert len(benign_paths) >= num, "良性数据长度不足!良性总长度: %d, 需求长度: %d" %(len(benign_paths), num)
    benign_paths = rd.sample(benign_paths, num)
    data = []
    for i,benign in enumerate(benign_paths):
        print("benign ", i)
        pe_data = extract_infos(benign)
        if pe_data is None:
            continue
        data.append(pe_data)
...
def collect_class_data(path, train_num_each=100, test_num_each=200, seed=1, train_targets=None, test_targets=None):
    '''
    从样本中获得单类缺省或者类型分割的数据
    :param path: 数据保存路径
    :param train_num_each: 训练集中，每个类别的抽取数量
    :param test_num: 测试集中每个类别的抽取数量
    ...
    #将良性文件分割给训练集合测试集
    train_data += benign_data[:len(train_data)]
    train_label += benign_label[:len(train_label)]
    test_data += benign_data[len(train_data):]
    test_label += benign_label[len(train_label):]
...
if __name__ == "__main__":
    ...
    #                   split=0.8)
    #对应mlExp2
    collect_class_data(path="D:/Few-Shot-Project/data/ExtClassOneDefault/virus/",
                        train_num_each=100,
                        test_num_each=200,
                       seed=5,
                       train_targets=['aworm','dos','backdoor2','exploit','trojan3','packed','rootkit','email','net-worm'],
                       test_targets=['virus'])
    # collect_save_data(path="D:/Few-Shot-Project/data/ExtClassOneDefault/backdoor/",
    #                   normalize=True,
    #                   num=200,
    #                   seed=15,
    #                   target_list=['backdoor2'],
    #                   split=-1)
    #用于产生所有的良性文件路径
