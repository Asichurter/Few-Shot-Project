...
1. 改用SGD优化，带有0.01系数方差

2. 学习率改为5000轮下降一次以保证学习率不会衰减得太快
...
1. 加入了注意力映射来生成类向量：利用类内的均值向量与类内向量的
平方距离的负值输入到softmax生成注意力系数，利用该注意力系数乘对应的
向量生成类向量
...
1. 使用了聚类数据集，同时改为了重叠池化和LeakyReLU

2. 通道数改为了1->32->64->128->256->512
...
- 9.7
考虑仿照Spatial Transformer论文中的结构，使用注意力机制实现的
变换来代替固定的池化
